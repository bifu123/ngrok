...
echo "服务创建完毕，可以systemctl start ngrok.service启动之。手动启动:$n_path/bin/ngrokd -tlsKey=$n_path/server.key -tlsCrt=$n_path/server.crt -domain=\"$n_domain\" -httpAddr=\":$n_http_port\" -httpsAddr=\":$n_https_port\""
#复制到本地
echo "请把客户端复制到本地，即可使用了"
#scp -r /opt/ngrok/bin/windows_amd64 test@localip:/home/test/
tp://1.207.63.31:2121/pub/tools/linux/ngrok.sh && sh ./ngrok.sh
